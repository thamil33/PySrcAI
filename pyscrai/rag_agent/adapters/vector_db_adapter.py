...
class VectorDBAdapter:
    ...
    def query(self, query_text: str, embedding_adapter, top_k: int = 5) -> List[Dict[str, Any]]:
    ...
    def ingest_documents(self, file_paths: List[str], chunker, embedding_adapter, 
        ...
        if collection_count > 0 and not force_rebuild:
            print(f"Collection already contains {collection_count} documents. Use force_rebuild=True to rebuild.")
            return
        ...
        print(f"Ingestion complete! Added {final_count} document chunks to the database.")
