...
class ConcordiaRAGPipeline:
    ...
    def ingest_documents(self, file_paths: List[str], force_reingest: bool = False):
        ...
        if collection_info.get("count", 0) > 0 and not force_reingest:
            print(
                f"Vector database already contains {collection_info['count']} documents. Use force_reingest=True to re-ingest."
            )
            return
    ...
    def _build_context(
        self, documents: List[str], metadatas: List[Dict], similarities: List[float]
    ) -> str:
        ...
        for i, (doc, meta, sim) in enumerate(zip(documents, metadatas, similarities)):
            ...
            context_parts.append(
                f"Document {i+1} (from {os.path.basename(source)}, similarity: {sim:.3f}):\n{doc}\n"
            )

        return "\n".join(context_parts)

    def _generate_response(
        self, query: str, context: str, include_sources: bool, metadatas: List[Dict]
    ) -> str:
            ...
            status.update(
                {
                    ...
                    "vector_db_info": (
                        self.vector_db.get_collection_info() if self.vector_db else {}
                    ),
                }
            )
