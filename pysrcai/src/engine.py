...
class SequentialEngine(SimulationEngine):
    ...
    def step(self):
        ...
        for agent in self.agents:
            if hasattr(agent, 'observe'):
                ...
                # Provide current inventory information if available
                if "inventory" in self.scenario_state and agent.name in self.scenario_state["inventory"]:
                    ...
                    if inventory:
                        ...
                        agent.observe(f"Your inventory: {', '.join(item_names)}")
                    else:
                        agent.observe("Your inventory is empty.")
            ...
            if hasattr(agent, 'act'):
                ...
                prompt = f"Step {self.turn+1}: What do you do? Available actions:\n"
                prompt += "- look [at/inside] [object/person]\n"
                prompt += "- take [item]\n"
                prompt += "- speak to [agent]\n"
                prompt += "- wait for now\n"
                if word_limit:
                ...
                if self.archon and hasattr(self.archon, 'act'):
                    ...
                    feedback_prompt = f"Describe what happens as a result of {agent.name}'s action: '{action}'. "
                    feedback_prompt += f"Then describe the current state of the {self.environment.active_location} and its objects."
        ...
        if self.archon and hasattr(self.archon, 'act'):
            ...
            if word_limit is None:
                word_limit = self.response_word_limit
            prompt = f"Step {self.turn+1}: Analyze this round of interactions."
            if word_limit:
                prompt += f" (Respond in {word_limit} words or less.)"
    ...
    def process_agent_action(self, agent, action):
        ...
        """
        # Check if the action mentions specific objects or interactions
        ...
        # Handle explicit "do nothing" action
        if "do nothing" in action_lower:
        ...
        if any(word in action_lower for word in ["speak", "talk", "say", "tell"]):
            for other_agent in self.agents:
                if other_agent != agent and other_agent.name.lower() in action_lower:
                    # Extract the message content after "say" or similar words
                    message_start = -1
                    for word in ["say", "says", "saying", "tell", "tells", "telling"]:
                        if word in action_lower:
                            message_start = action_lower.find(word) + len(word)
                            break
                    
                    if message_start > -1:
                        message = action[message_start:].strip(' "\'')
                        ...
                        return f"{agent.name} attempts to speak to {other_agent.name} but doesn't say anything specific."
                    
                    interaction_found = True
                    break
        
        # Check for examining/looking at objects
        if any(word in action_lower for word in ["look", "examine", "observe", "inspect"]):
            # Check for specific object references
            for obj_id in self._get_all_object_ids():
                if obj_id.lower() in action_lower or self._get_object_name(obj_id).lower() in action_lower:
            ...
            # Look at current location if no specific object
            if not interaction_found and self.environment.active_location:
                result = self.environment.process_interaction(
                    agent_id=agent.name,
                    action="examine",
        ...
        elif any(word in action_lower for word in ["search", "look inside", "check inside"]):
            for obj_id in self._get_all_object_ids():
                if obj_id.lower() in action_lower or self._get_object_name(obj_id).lower() in action_lower:
                    result = self.environment.process_interaction(
                        agent_id=agent.name,
                    ...
                    # If search found items, make them visible in the environment
                    if result.get("success", False) and "found_items" in result:
                        for item_id in result["found_items"]:
                            if item_id in self.environment.items:
                                # Update item's location property
        ...
        return result.get("observation", result.get("message", "You performed an action."))
